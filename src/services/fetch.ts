...
export const fetch_same_types_pokemon_data = async (typesUrl: TypesData[]) : Promise<PokemonData[]> => {
    try {

        const rawSameTypeData = await Promise.all(typesUrl.flatMap(async (e: any) => {
        ...
        return Array.from(new Set([].concat(...rawSameTypeData).map(item => JSON.stringify(item)))).map(item => JSON.parse(item));
    } catch(err: any) {
      console.log(err);
    }
}
...
export const fetch_evolution_chain = async (speciesUrl: string, base_pokemon: PokemonData) : Promise<EvoData> => {
    try {
        ...
        if(res0?.data?.evolution_chain?.url)
        {
            ...
            if(res1?.data.chain) {
                ...
                while(evoData.length > 0) {
                    if(evofrom) {
                        evofrom = !evoData.find(pokemon => pokemon.species.name == base_pokemon.name)
