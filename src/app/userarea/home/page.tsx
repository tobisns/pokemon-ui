			...
			const getData = setTimeout(async () => {
				try {
					const updatedPokemonArray =
						await get_filtered_pokemon(filter)
					setIndex(0)
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 grid-rows-3 md:grid-rows-2 gap-4 h-5/6 w-4/5 mx-auto">
						{pokemonData &&
							pokemonData.map((pokemon) => (
								<PokemonWidget
									className="bg-gray-400 border-double border-2 border-white"
									key={pokemon.name}
									name={pokemon.name}
								/>
							))}
	...
	return (
		<div>
			<div className="fixed w-full bg-black sm:pt-12 pb-8 p-4 sm:p-12 z-10 flex">
				...
				<select
					className="bg-white-700 text-black p-2 rounded w-5/6 sm:w-96 ml-2"
